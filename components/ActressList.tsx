...
const ActressList = () => {
  ...
  const [allActressState, setAllActressState] = useState<ActressType[]>(allActress)
  ...
  const actressCheckedNum: number = allActressState.filter(actress => actress.isChecked).length
  const percentage: number = Math.round(actressCheckedNum / actressNum * 10000) / 100

  return (
	<>
	  <p className="text-xl text-center m-2">スカウト率 {percentage}% ( {actressCheckedNum}/{actressNum} )</p>

      <div className="max-w-3xl mx-auto my-4 flex flex-wrap justify-center gap-2">
	    {allActressState.map((actress) => (
		  <ActressBox key={actress.id}
		              actress={actress}
